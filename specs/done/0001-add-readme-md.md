---
card_id: 6a297411fe0d24e5425a9963
card_short: 0001
...
`kb-rag` is a lightweight, framework-agnostic RAG package for Node.js that runs fully local with no external embedding API. It was extracted from DataClara, a multi-tenant SaaS platform for MercadoLibre sellers. The package is published to npm as `@jonathanleonel/kb-rag`. There is currently no documentation at the repo root — the README.md file is missing entirely.

## Objective

Create `README.md` at the repository root with the exact content approved by the product owner. No other files are to be modified.

## Constraints

- The file content must be reproduced verbatim — no rewording, reformatting, or structural changes
- The npm package name `@jonathanleonel/kb-rag` must appear exactly as-is
- Only `README.md` at the repo root is touched in this task

## Acceptance criteria

- [ ] `README.md` exists at the root of the repository
- [ ] The file content matches the approved text exactly
- [ ] The file is committed to `main` with a descriptive commit message (e.g. `docs: add README`)
